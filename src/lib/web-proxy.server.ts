...
export async function handleProxy(request: Request, targetHref: string): Promise<Response> {
  ...
  try {
    upstream = await fetch(target.href, { redirect: "follow", headers: reqHeaders });
  } catch {
